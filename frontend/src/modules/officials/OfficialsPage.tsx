...
import {
  ...
  OFFICIAL_VERIFICATION_STATUS_OPTIONS,
  formatExperience,
  joinLabels,
  officialRoleLabel,
  officialStatusClass,
...
export default function OfficialsPage() {
  ...
  const [result, setResult] = useState<Awaited<ReturnType<typeof api.officials.list>> | null>(null)
  ...
  const [verificationFilter, setVerificationFilter] = useState<OfficialVerificationStatus | ''>('')
  const [statusFilter, setStatusFilter] = useState<OfficialStatus | ''>('')
  const [roleFilter, setRoleFilter] = useState<OfficialRole | ''>('')
  ...
  useEffect(() => {
    ...
    api.officials
      .list(token, {
        ...
        officialStatus: statusFilter || undefined,
        role: roleFilter || undefined,
      })
      .then(setResult)
      .catch(() => setError('Daftar wasit tidak dapat dimuat.'))
      .finally(() => setLoading(false))
  }, [revision, token, page, search, scopeDistrictId, verificationFilter, statusFilter, roleFilter])

  // Statistik mengikuti wilayah yang sedang dilihat, bukan hasil filter tabel.
  useEffect(() => {
    if (!token) return
    api.officials.stats(token, scopeDistrictId).then(setStats).catch(() => setStats(null))
  ...
  useEffect(() => {
    ...
    setFormError('')
    api.forms
      .list(token, { districtId: isAllRegions ? undefined : workspace?.id, type: 'OFFICIAL_REGISTRATION' })
  ...
  const copyLink = async () => {
    if (!form) return
    await navigator.clipboard?.writeText(`${window.location.origin}/form/official/${form.publicToken}`)
    setCopied(true)
  ...
  return (
    <div className="mx-auto w-full max-w-9xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 md:text-3xl">Master Wasit</h1>
          <p className="mt-2 text-sm text-gray-500">Data wasit tenis PELTI Bali di workspace ini.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <ReviewInboxButton to="/officials/review" label="Review Wasit" count={pendingReview} />
          {canManageForm && (
            <button
          ...
          <Link className="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800" to="/officials/new">
            Tambah Wasit
          </Link>
        </div>
      </div>
      ...
      <section className="rounded-xl bg-white shadow-xs dark:bg-gray-800">
        <PortalToolbar
          search={{
            ...
            placeholder: 'Cari nama / NIK wasit',
          }}
          onReset={() => {
            setPage(1)
            setDistrictFilter('')
        ...
        >
          ...
          <select
            ...
            onChange={(event) => {
              setPage(1)
              setRoleFilter(event.target.value as OfficialRole | '')
            }}
          >
          ...
          <select
            ...
            onChange={(event) => {
              setPage(1)
              setStatusFilter(event.target.value as OfficialStatus | '')
            }}
          >
          ...
          <select
            ...
            onChange={(event) => {
              setPage(1)
              setVerificationFilter(event.target.value as OfficialVerificationStatus | '')
            }}
          >
            <option value="">Semua verifikasi</option>
            {OFFICIAL_VERIFICATION_STATUS_OPTIONS.map((option) => (
              <option key={option.value} value={option.value}>
                {option.label}
              </option>
            ))}
        ...
        {loading ? (
          <p className="p-8 text-sm text-gray-500">Memuat wasit…</p>
        ) : !result?.data.length ? (
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-200">Belum ada data wasit</p>
            <p className="mt-1 text-sm text-gray-500">Wasit yang sudah disetujui admin akan tampil di sini.</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto p-3">
              <table className="w-full table-auto dark:text-gray-300">
                <thead className="bg-gray-50 text-xs uppercase text-gray-400 dark:bg-gray-700/50 dark:text-gray-500">
                  <tr>
                    {['Kode', 'Nama Wasit', 'Distrik', 'Peran', 'Pengalaman', 'Turnamen', 'Status', 'Verifikasi', 'Aksi'].map((heading, index) => (
                ...
                <tbody className="divide-y divide-gray-100 text-sm font-medium dark:divide-gray-700/60">
                  {result.data.map((official) => (
                    <tr key={official.id}>
                      ...
                      <td className="p-2 font-mono text-xs text-gray-500">{official.officialCode}</td>
                      <td className="p-2 text-gray-800 dark:text-gray-100">{official.fullName}</td>
                      <td className="p-2 text-center text-gray-500">{official.district.name}</td>
                      <td className="p-2 text-center text-gray-500">
                        <div className="max-w-[12rem] truncate">{joinLabels(official.roles, officialRoleLabel)}</div>
                      </td>
                      <td className="p-2 text-center text-gray-500">{formatExperience(official.officiatingSince)}</td>
                      {/* §5 — angka ini dihitung server dari riwayat turnamen. */}
                      <td className="p-2 text-center text-gray-500">{official._count?.tournaments ?? 0}</td>
                      <td className="p-2 text-center">
                        <InlineSelect label={`Ubah status aktif ${official.fullName}`} value={official.officialStatus} options={OFFICIAL_STATUS_OPTIONS} className={officialStatusClass[official.officialStatus]} disabled={!canManageForm} onSave={async (value) => { if (!token) return; await api.officials.update(token, official.id, { officialStatus: value }); setResult((current) => current ? { ...current, data: current.data.map((row) => row.id === official.id ? { ...row, officialStatus: value as typeof row.officialStatus } : row) } : current); setRevision((value) => value + 1) }} />
                      </td>
                      <td className="p-2 text-center">
                        <InlineSelect label={`Ubah status verifikasi ${official.fullName}`} value={official.verificationStatus} options={OFFICIAL_VERIFICATION_STATUS_OPTIONS} className={officialVerificationStatusClass[official.verificationStatus]} disabled={!canManageForm} onSave={async (value) => { if (!token) return; await api.officials.verify(token, official.id, { verificationStatus: value as OfficialVerificationStatus }); setResult((current) => current ? { ...current, data: current.data.map((row) => row.id === official.id ? { ...row, verificationStatus: value as typeof row.verificationStatus } : row) } : current); setRevision((value) => value + 1) }} />
                      </td>
                      <td className="p-2 text-center">
                        <button
      ...
      {stats && <DistrictBreakdown title="Jumlah Wasit per Distrik" rows={stats.byDistrict} />}

      {manageOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Kelola form pendataan wasit">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-gray-200 bg-white p-6 shadow-lg dark:border-gray-700/60 dark:bg-gray-800">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-gray-400">Form Pendataan Wasit</p>
            ...
            {formLoading ? (
              ...
              <>
                ...
                <div className="mt-6 flex justify-end gap-3 border-t border-gray-100 pt-5 dark:border-gray-700/60">
                  <button
                    ...
                    onClick={() => window.open(`/form/official/${form.publicToken}`, '_blank', 'noopener,noreferrer')}
                  >
                    Buka Form
                  </button>
                  {form.status === 'ACTIVE' ? (
              ...
              <>
                <p className="mt-5 text-sm text-gray-500">
                  Form pendataan wasit disiapkan otomatis untuk workspace ini. Pilih satu workspace distrik terlebih dahulu bila belum.
                </p>
                {formError && <p className="mt-3 text-sm text-red-600">{formError}</p>}
