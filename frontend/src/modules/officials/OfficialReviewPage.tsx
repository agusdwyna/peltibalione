...
import { formatExperience, joinLabels, officialRoleLabel, officialStatusLabel } from '../../types/official'
...
export default function OfficialReviewPage() {
  ...
  const [result, setResult] = useState<Awaited<ReturnType<typeof api.officialSubmissions.list>> | null>(null)
  ...
  useEffect(() => {
    ...
    if (!scopeDistrictId) {
      setResult(null)
      setError('Pilih satu workspace distrik untuk melihat antrean pengajuan wasit.')
      setLoading(false)
      return
    }
    setLoading(true)
      ...
      .catch((reason) =>
        setError(
          reason instanceof ApiError && reason.code === 'DISTRICT_CONTEXT_REQUIRED'
            ? 'Pilih satu workspace distrik untuk melihat antrean pengajuan wasit.'
            : 'Pengajuan wasit tidak dapat dimuat.',
        ),
      )
  ...
  return (
    <div className="mx-auto w-full max-w-9xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <Link className="mb-4 inline-block text-sm text-gray-500 hover:text-gray-800 dark:hover:text-gray-200" to="/officials">
          ← Kembali ke daftar wasit
        </Link>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 md:text-3xl">Review Wasit</h1>
        <p className="mt-2 text-sm text-gray-500">
          Tinjau pengajuan pendataan wasit, lalu setujui dengan status verifikasi, atau tolak dengan catatan.
        </p>
      </div>
      ...
      <section className="rounded-xl bg-white shadow-xs dark:bg-gray-800">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-200">Belum ada pengajuan wasit</p>
            ...
            <div className="overflow-x-auto p-3">
              <table className="w-full table-auto dark:text-gray-300">
                <thead className="bg-gray-50 text-xs uppercase text-gray-400 dark:bg-gray-700/50 dark:text-gray-500">
                  <tr>
                    {['Wasit', 'Peran', 'Pengalaman', 'Sertifikat', 'Turnamen', 'Distrik', 'Status', 'Aksi'].map((heading, index) => (
                  ...
                  {result.data.map((submission) => (
                    <tr key={submission.id}>
                      ...
                      <td className="p-2 text-center text-gray-500">
                        <div className="max-w-[12rem] truncate">{joinLabels(submission.roles, officialRoleLabel)}</div>
                      </td>
                      <td className="p-2 text-center text-gray-500">{formatExperience(submission.officiatingSince)}</td>
                      <td className="p-2 text-center text-gray-500">{submission._count?.certificates ?? 0}</td>
                      <td className="p-2 text-center text-gray-500">{submission._count?.tournaments ?? 0}</td>
                      ...
                      <td className="p-2 text-center">
                        <div className="flex flex-wrap justify-center gap-1">
                          ...
                          <span className="rounded-full bg-gray-100 px-2 py-1 text-[11px] font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300">
                            {officialStatusLabel(submission.officialStatus)}
                          </span>
                        </div>
                      </td>
                      <td className="p-2 text-center">
                        <button
                          ...
                          onClick={() => navigate(`/officials/review/${submission.id}`)}
                        >
                          Tinjau
                        </button>
                      </td>
