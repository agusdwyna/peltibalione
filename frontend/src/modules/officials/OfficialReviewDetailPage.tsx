...
export default function OfficialReviewDetailPage() {
  ...
  const [submission, setSubmission] = useState<OfficialSubmissionDetail | null>(null)
  ...
  const review = async (action: 'LINK' | 'REJECT') => {
    ...
    setError('')
    try {
      await api.officialSubmissions.review(token, submission.id, {
        action,
      ...
      navigate('/officials/review', { replace: true })
    } catch (reason) {
  ...
  return (
    <div className="mx-auto w-full max-w-9xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* §16 — NIK ganda harus terlihat sebelum admin menyetujui. */}
      {submission.duplicateOfOfficial && (
        <div className="mb-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800" role="alert">
          NIK ini sudah terdaftar atas nama <strong>{submission.duplicateOfOfficial.fullName}</strong> (
          {submission.duplicateOfOfficial.officialCode}). Pengajuan tidak dapat disetujui sebelum datanya diperbaiki.
        </div>
      )}
      ...
      <section className="rounded-xl bg-white p-6 shadow-xs dark:bg-gray-800 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-100 pb-5 dark:border-gray-700/60">
          <div>
            <p className="text-xs uppercase tracking-wide text-gray-400">Detail pengajuan wasit</p>
        ...
        <div className="mt-7">
          <OfficialSpecs official={submission} />
        </div>
          ...
          {rejecting ? (
            ...
            <>
              ...
              <button
                ...
                disabled={processing || Boolean(submission.duplicateOfOfficial)}
                onClick={() => review('LINK')}
              >
                {processing ? 'Memproses…' : 'Setujui & buat master wasit'}
              </button>
            </>
          )}
        </div>
