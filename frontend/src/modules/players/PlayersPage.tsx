...
export default function PlayersPage() {
  ...
  // Lencana antrean review — jumlah pendaftaran pemain yang belum diproses.
  useEffect(() => {
  ...
  }, [revision, page, token, workspace?.id, isAllRegions, districtFilter, search, genderFilter, ageGroupFilter])
    ...
    <div className="mx-auto w-full max-w-9xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <ReviewInboxButton to="/verification" label="Review Pendaftaran" count={pendingReview} />
      ...
      <section className="rounded-xl bg-white shadow-xs dark:bg-gray-800">
        ...
        {loading ? <p className="p-8 text-sm text-gray-500">Memuat pemain…</p> : !result?.data.length ? <div className="p-10 text-center"><p className="text-sm font-medium text-gray-700 dark:text-gray-200">Belum ada data pemain</p><p className="mt-1 text-sm text-gray-500">Pemain yang sudah diverifikasi di workspace ini akan tampil di sini.</p></div> : <>
          <div className="overflow-x-auto p-3"><table className="w-full table-auto dark:text-gray-300"><thead className="bg-gray-50 text-xs uppercase text-gray-400 dark:bg-gray-700/50 dark:text-gray-500"><tr>{['ID', 'Nama pemain', 'Kelompok umur', 'Kabupaten', 'Status', 'Aksi'].map((heading, index) => <th className="p-2" key={heading}><div className={index === 0 ? 'text-left font-semibold' : 'text-center font-semibold'}>{heading}</div></th>)}</tr></thead><tbody className="divide-y divide-gray-100 text-sm font-medium dark:divide-gray-700/60">{result.data.map((player) => <tr key={player.id}><td className="p-2 font-mono text-xs text-gray-500">{player.playerCode}</td><td className="p-2 text-gray-800 dark:text-gray-100">{player.fullName}</td><td className="p-2 text-center text-gray-500">{formatAgeGroup(player.ageGroup, player.gender)}</td><td className="p-2 text-center text-gray-500">{player.district.name}</td><td className="p-2 text-center"><InlineSelect label={`Ubah status ${player.fullName}`} value={player.status} options={Object.entries(statusLabel).map(([value, label]) => ({ value, label }))} className={statusClass[player.status]} disabled={!canManageForm} onSave={async (value) => { if (!token) return; await api.players.updateStatus(token, player.id, value as 'VERIFIED' | 'ACTIVE' | 'INACTIVE'); setResult((current) => current ? { ...current, data: current.data.map((row) => row.id === player.id ? { ...row, status: value as typeof row.status } : row) } : current); setRevision((value) => value + 1) }} /></td><td className="p-2 text-center"><button className="text-sm font-medium text-violet-600 hover:text-violet-700" onClick={() => navigate(`/players/${player.id}`)}>Detail</button></td></tr>)}</tbody></table></div>
