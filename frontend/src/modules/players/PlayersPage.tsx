...
export default function PlayersPage() {
  ...
  const [title, setTitle] = useState('Form Pendaftaran Pemain')
  ...
  useEffect(() => {
    if (!token) return
    setLoading(true)
    api.players.list(token, { page, q: search || undefined, districtId: isAllRegions ? districtFilter || undefined : workspace?.id })
      .then(setResult).catch(() => setError('Daftar pemain tidak dapat dimuat.')).finally(() => setLoading(false))
  }, [page, token, workspace?.id, isAllRegions, districtFilter, search])

  useEffect(() => {
    if (!token) return
    setFormLoading(true)
    api.forms.list(token).then((items) => setForm(items[0] ?? null)).catch(() => setFormError('Form pendaftaran tidak dapat dimuat.')).finally(() => setFormLoading(false))
  }, [token, workspace?.id])
  ...
  const createForm = async (event: FormEvent) => {
    ...
    setFormError('')
    try {
      const created = await api.forms.create(token, { title, description, ...(isCentral ? { districtId: workspace?.id } : {}) })
      setForm(created); setTitle('Form Pendaftaran Pemain'); setDescription('')
    } catch { setFormError('Form tidak dapat dibuat. Pastikan belum ada form aktif di workspace ini.') }
  }
  ...
  return (
    <div className="mx-auto w-full max-w-9xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="rounded-xl bg-white shadow-xs dark:bg-gray-800">
        <PortalToolbar search={{ value: query, onChange: setQuery, onSubmit: () => { setPage(1); setSearch(query.trim()) }, placeholder: 'Cari nama pemain' }}>
        ...
        {loading ? <p className="p-8 text-sm text-gray-500">Memuat pemain…</p> : !result?.data.length ? <div className="p-10 text-center"><p className="text-sm font-medium text-gray-700 dark:text-gray-200">Belum ada data pemain</p><p className="mt-1 text-sm text-gray-500">Pemain yang sudah diverifikasi di workspace ini akan tampil di sini.</p></div> : <>
          <div className="overflow-x-auto p-3"><table className="w-full table-auto dark:text-gray-300"><thead className="bg-gray-50 text-xs uppercase text-gray-400 dark:bg-gray-700/50 dark:text-gray-500"><tr>{['Pemain', 'ID', 'NIK', 'Kabupaten', 'Klub', 'Status', 'Aksi'].map((heading, index) => <th className="p-2" key={heading}><div className={index === 0 ? 'text-left font-semibold' : 'text-center font-semibold'}>{heading}</div></th>)}</tr></thead><tbody className="divide-y divide-gray-100 text-sm font-medium dark:divide-gray-700/60">{result.data.map((player) => <tr key={player.id}><td className="p-2 text-gray-800 dark:text-gray-100">{player.person.fullName}</td><td className="p-2 text-center text-gray-500">{player.playerCode}</td><td className="p-2 text-center font-mono text-gray-500">{player.person.nik ? maskNik(player.person.nik) : '—'}</td><td className="p-2 text-center text-gray-500">{player.district.name}</td><td className="p-2 text-center text-gray-500">{player.club?.name ?? '—'}</td><td className="p-2 text-center"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass[player.status] ?? 'bg-gray-100 text-gray-600'}`}>{statusLabel[player.status] ?? player.status}</span></td><td className="p-2 text-center"><button className="text-sm font-medium text-violet-600 hover:text-violet-700" onClick={() => navigate(`/players/${player.id}`)}>Detail</button></td></tr>)}</tbody></table></div>
